...
このリポジトリで作業する AI は、まず `AI.md` を参照してください。

**Rule:** In each command, **define -> use**. Do **not** escape `$`. Use generic `'path/to/file.ext'`.
**Encoding:** Always create and update text files as UTF-8 without BOM. Prefer ASCII in instructions and examples unless non-ASCII is required.
