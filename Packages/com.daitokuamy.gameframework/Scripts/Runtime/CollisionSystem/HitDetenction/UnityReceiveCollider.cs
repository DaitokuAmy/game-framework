...
namespace GameFramework.CollisionSystem {
    /// <summary>
    /// UnityのCapsuleColliderベースのReceiveCollider
    /// </summary>
    public sealed class UnityReceiveCollider : IReceiveCollider {
        private readonly CapsuleCollider _collider;

        /// <inheritdoc/>
        Vector3 IReceiveCollider.Start => GetWorldCapsule().start;
        /// <inheritdoc/>
        Vector3 IReceiveCollider.End => GetWorldCapsule().end;
        /// <inheritdoc/>
        float IReceiveCollider.Radius => GetWorldCapsule().radius;

        /// <summary>
        /// コンストラクタ
        /// </summary>
        public UnityReceiveCollider(CapsuleCollider collider) {
            _collider = collider;
        }
