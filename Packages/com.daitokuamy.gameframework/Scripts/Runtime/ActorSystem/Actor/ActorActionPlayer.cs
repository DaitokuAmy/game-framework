...
namespace GameFramework.ActorSystem {
    ...
    /// </summary>
    public sealed class ActorActionPlayer : IDisposable {
        /// <summary>
        /// アクターアクション再生中情報
        /// </summary>
        internal class PlayingInfo {
            private readonly ActorActionPlayer _player;
            ...
            /// コンストラクタ
            /// </summary>
            public PlayingInfo(ActorActionPlayer player, Coroutine coroutine, IActorAction action, IActorActionHandler handler) {
                _player = player;
            ...
            public void Cancel() {
                ...
                _player._coroutineRunner.StopCoroutine(_coroutine);
                _coroutine = null;
                _handler.Cancel(_action);
                _handler.Exit(_action);
                _player.OnFinishedAction(_action, _handler.GetOutBlendDuration(_action));
                _handler = null;
                _action = null;
        ...
        public ActorActionHandle PlayAction(IActorAction action) {
            ...
            _playingInfo = new PlayingInfo(this, coroutine, action, handler);
        ...
        /// アクション終了通知
        /// </summary>
        private void OnFinishedAction(IActorAction action, float duration) {
            FinishedEvent?.Invoke(action, duration);
        }
    }
}
