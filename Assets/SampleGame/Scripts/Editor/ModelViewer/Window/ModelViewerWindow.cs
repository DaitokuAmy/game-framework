...
namespace SampleGame.ModelViewer.Editor {
    ...
    public partial class ModelViewerWindow : EditorWindow {
        ...
        private class SearchableList<T> {
            ...
            public void OnGUI(IList<T> items, Func<T, string> itemToName, Action<T, int> onGUIElement, params GUILayoutOption[] options) {
                using (new EditorGUILayout.VerticalScope("Box", options)) {
                    ...
                    var filteredItems = items.Select((x, i) => (x, i)).Where(pair => {
        ...
        private class Foldout {
            ...
            /// コンストラクタ
            /// </summary>
            public Foldout(string label, bool defaultOpen = true) {
                _label = label;
                _open = defaultOpen;
            }
            ...
            /// GUI描画
            /// </summary>
            /// <param name="onGUI">項目GUI描画</param>
            /// <param name="options">LayoutOption</param>
            public void OnGUI(Action onGUI, params GUILayoutOption[] options) {
        ...
        private void CreatePanels() {
            // パネルの生成
            void CreatePanel<T>() where T : PanelBase, new() {
                var panel = new T();
        ...
        private void PlayScene() {
            ...
            var guids = AssetDatabase.FindAssets("t:scene model_viewer");
            if (guids.Length <= 0) {
                Debug.LogWarning("Not found model_viewer scene.");
                return;
            }
